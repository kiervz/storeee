...
const Header = (props: {
  ...
  return (
    <header className="sticky top-0 z-50 flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none">
      <div className="flex flex-grow items-center justify-between py-2 px-4 shadow md:px-6 2xl:px-11">
        <div className="flex items-center gap-2 sm:gap-4 lg:hidden">
          ...
          <Link className="block flex-shrink-0 lg:hidden" href="/">
            <p className="text-black text-sm font-semibold">Kvey Store</p>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3 2xsm:gap-7">
          {session.data?.user && <UserDropdown user={session?.data.user} />}
        </div>
      </div>
    </header>
  );
